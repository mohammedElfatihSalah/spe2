...
def compute_average_metric(feedback, aux, hint_name, options):
  ''' 
  a method to compute the average accuracy for a given hint,
  the hint must be of type  >> MASK << , and location >> GRAPH << 
  '''
  split, hints_to_log, step, available_splits = options['split'], options['hints'], options['step'], options['available_splits']
  ...
  # get file name based on the split and the step
  #filename = split + '-- step [' + str(step) + ']' + '.txt'
  #filepath = os.path.join(split, filename)
  # End

  # Create the file
  #created_file = open(filepath, 'a')
  #
  ...
  #created_file.write('Inputs:' + '\n')
  inputs = feedback.features[0]
  #for inp in inputs:
    #if inp.name == 'key':
     # created_file.write('\t' + inp.name + ": " + str(inp.data[0,:].argmax(-1)) + '\n')
  # End
  ...
  # predicted:
    ...
    # j:
    # s:
  ground_truth_hints = feedback.features[1]
 #created_file.write('hint ground truth:' + '\n')
  #for h in ground_truth_hints:
   # if h.name in hints_to_log:
      #created_file.write('\t' + h.name + ": " + str(h.data[:,0,:].argmax(-1)) + '\n')
  
  
  #predicted_hints_dict = {}
  #for name in hints_to_log:
    #val = [aux[0][i][name] for i in range(len(aux[0]))]
    # -- cast to numpy array
    #predicted_hints_dict[name] = np.array(val)
  #created_file.write('hint predicted:' + '\n')
  #for hintname, data in predicted_hints_dict.items():
    #created_file.write('\t' + hintname + ': '+ str(data[:,0,:].argmax(-1)))
...
def main(unused_argv):
  ...
  'step':0,
  'split':'train',
  'available_splits':['train', 'val', 'test', 'train [m]', 'val [m]', 'test [m]'],
}
...
#   print("val")
  ...
  def evaluate(step, model, feedback, extras=None, verbose=False, test=False, split='train'):
  ...
  for step in range(FLAGS.train_steps):
    ...
    if step % FLAGS.log_every == 0:
      ...
      val_stats, accuracy, shift, final = evaluate(
          step, model, val_feedback, verbose=FLAGS.verbose_logging, test=False, split='val')
      ...
      #   wandb.log({'train match [manual]':train_stats['match'], 
      #   'train average accuracy [manual]':accuracy,
      #   'train average shift [manual]':shift,
      #   'train final accuracy [manual]':final,
      #   })
      ...
      #   val_stats, accuracy, shift, final = evaluate(
      #     step, model, val_feedback, verbose=FLAGS.verbose_logging, test=True, split='val [m]')
      #   wandb.log({
      #     'validation match [manual]':val_stats['match'],
      #     'validation average accuracy [manual]':accuracy,
      #      'validation average shift [manual]': shift, 
      #      'final validation accuracy [manual]':final})
      
      # if step % 100 == 0:
      #   test_sampler, _ = clrs.clrs21_test(FLAGS.algorithm)
      #   test_feedback = test_sampler.next()
      #   test_stats, accuracy, shift, final = evaluate(
      #     step, model, test_feedback, verbose=FLAGS.verbose_logging, test=True, split='test [m]')
      #   wandb.log({
      #     'test final accuracy [manual]': final,
      #     'test match [manual]':test_stats['match'],
      #     'test average accuracy [manual]':accuracy,
      #     'test average shift [manual]': shift
      #   })
